...
if [[ -e ${image} ]]; then
  ...
  maas admin boot-resources create name='custom/$(echo "${image}"|sed -e "s/\..*//")' title='$(echo "${image}" | sed -e "s/\..*//;s/\-/ /;s/\b\(.\)/\u\1/g") Installer' architecture='amd64/generic' filetype='tgz' sha256='$(sha256sum "${image}" | cut -d ' ' -f1)' size='$(stat -c'%s' "${image}")' base_image='ubuntu/jammy' content@='${image}'
EOF
fi
done
